...
namespace mattatz.TeddySystem {
    ...
    public class Teddy {
		...
		*/
        ...
        // z-offset so skeleton is drawn in front of the mesh (camera looks along -Z)
        const float SkeletonZ = 0.001f;

        /// <summary>
        /// Returns a flat list of (start, end) bone segments from the chordal axis.
        /// Pruned chords are skipped but traversal continues through them so all
        /// surviving spine edges are collected. Call after Build().
        /// </summary>
        public List<(Vector3, Vector3)> GetSkeletonBones() {
            var result = new List<(Vector3, Vector3)>();
            if (chord != null) CollectSkeletonBones(chord, null, result);
            return result;
        }

        void CollectSkeletonBones(Chord2D current, Chord2D from, List<(Vector3, Vector3)> result) {
            // Only emit a bone segment if this chord was NOT pruned away
            if (!current.Pruned) {
                Vector3 src = ToSkeletonPoint(current.Src);
                Vector3 dst = ToSkeletonPoint(current.Dst);
                result.Add((src, dst));
            }
            // Always recurse so we don't miss non-pruned children of pruned nodes
            foreach (Chord2D next in current.Connection) {
                if (next != from) CollectSkeletonBones(next, current, result);
            }
        }

        Vector3 ToSkeletonPoint(Vertex2D v) {
            // Draw skeleton at a fixed z slightly in front of the mesh contour
            return new Vector3(v.Coordinate.x, v.Coordinate.y, SkeletonZ);
        }
