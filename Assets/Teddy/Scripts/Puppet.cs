	...
	public class Puppet : MonoBehaviour {
		...
		void DrawPathWithSoftBrush(List<Vector3> path, Camera cam, float dabSize, float spacing, bool loopClosed) {
			for (int i = 0; i < path.Count - 1; i++) {
				...
				if (p0.z > 0f && p1.z > 0f) {
					Vector2 s0 = new Vector2(p0.x, Screen.height - p0.y);
					Vector2 s1 = new Vector2(p1.x, Screen.height - p1.y);
		...
		void Update () {
			...
			if (enablePhysics) {
				...
				// ── Track and apply whole-object transform changes (e.g. from UI dragging) ─
				if (transform.localToWorldMatrix != msPrevLocalToWorld) {
					Matrix4x4 worldToLocal = msPrevLocalToWorld.inverse;
					Matrix4x4 localToWorld = transform.localToWorldMatrix;
					for (int i = 0; i < worldJoints.Count; i++) {
						Vector3 localPos = worldToLocal.MultiplyPoint3x4(worldJoints[i]);
						worldJoints[i] = localToWorld.MultiplyPoint3x4(localPos);
					}
					msPrevLocalToWorld = transform.localToWorldMatrix;
				}

				// ── Handle pinned / dragged / animated joints (override) ─────────
				for (int i = 0; i < worldJoints.Count; i++) {
					if (pinnedJoints.Contains(i)) {
						worldJoints[i] = transform.TransformPoint(restLocalPositions[i]);
						if (msVelocities != null) { msVelocities[3*i]=0; msVelocities[3*i+1]=0; msVelocities[3*i+2]=0; }
						continue;
					}
				...
				if (boneIndices != null && boneIndices.Count > 0 && worldJoints != null && worldJoints.Count > 0) {
					// Use fixed timestep so the precomputed Cholesky stays consistent
					float h = fixedTimestep;
					// Rebuild matrices when topology/pins/params/dragging change OR timestep drifts
					if (msMatricesDirty
					    || Mathf.Abs(msLastH - h) > 1e-6f
					    || Mathf.Abs(msLastStiffness - springStiffness) > 1e-4f
					    || Mathf.Abs(msLastShapeStiffness - shapeStiffness) > 1e-4f
					    || Mathf.Abs(msLastMass - nodeMass) > 1e-6f
					    || msLastDraggingJoint != draggingJoint)
						BuildMassSpringMatrices(h);
					MassSpringStep(h);
				}
				...
				// Zero velocities so solver restarts clean when physics is re-enabled
				if (msVelocities != null) System.Array.Clear(msVelocities, 0, msVelocities.Length);
			}
		}
		...
		public void SetupSkeleton(List<(Vector3, Vector3)> bones) {
			...
			foreach (var j in joints) {
				worldJoints.Add(transform.TransformPoint(j));
			}
